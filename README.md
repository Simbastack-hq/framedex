...
photos_keywords:
  - sunset
  - drone
live_photo: true        # only present when true
photos_edited: true     # only present when Photos has edits on this clip
```

`file:` is overridden to the user-meaningful camera filename (the on-disk name inside `.photoslibrary` is a UUID like `B627DB90-...mp4`; we keep that under `photos_uuid:` and `path:` for traceability).
