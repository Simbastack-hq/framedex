...
def main() -> int:
    ...
    for p in videos[:10]:
        ...
        print(
            f"  [{tag:6s}] {p.uuid[:8]}  {name:30s}  {date}  "
    ...
    if on_disk_count == 0:
        print(
            "⚠  All videos are iCloud-only. Two options to process them:\n"
            "   1. Photos → Settings → iCloud → 'Download Originals to this Mac'\n"
            "      (slow first sync but then fdx-photos works without --download)\n"
            "   2. fdx-photos --download (per-clip PhotoKit fetch — needs the\n"
            "      terminal to have Photos permission in System Settings)"
        )
    elif icloud_count == 0:
        print(
            f"✓ All {on_disk_count} videos are local. fdx-photos can process them "
            "directly without --download."
        )
    else:
        print(
            f"✓ {on_disk_count} videos can be indexed immediately without --download.\n"
